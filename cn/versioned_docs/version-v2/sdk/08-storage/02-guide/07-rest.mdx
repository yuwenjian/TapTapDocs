...
参数 | 说明 | 示例
---|---|---|
`i` | **忽略大小写** | `{"$regex":"single", "$options":"i"}` 将匹配：`{ "_id" : 100, "title" : "Single line description." }`
`m` | **多行匹配**<br/>比如文本中包含了换行符 `\n` | `{"$regex":"^S", "$options":"m"}`（以大写字母 S 开头）将匹配：`{ "_id" : 100, "title" : "Single line description." }` 和 `{ "_id" : 101, "title" : "First line\nSecond line" }`
...
`s` | **允许 `.` 匹配任意字符和换行** | `{"$regex":"m.*line", "$options":"si"}` 将匹配：`{ "_id" : 102, "title" : "Many spaces before     line" }` 和 `{ "_id" : 103, "title" : "Multiple\nline description" }`
