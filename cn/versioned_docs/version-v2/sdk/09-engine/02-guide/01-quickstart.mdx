...
打开浏览器访问 [http://localhost:3000](http://localhost:3000) 会显示如下内容：
...
用浏览器打开 [http://localhost:3000/todos](http://localhost:3000/todos) ，然后在输入框输入 「点个外卖」并点击 「新增」，可以看到 Todo 列表新增加了一行。
