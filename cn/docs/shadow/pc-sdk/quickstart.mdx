...
int main() {
    ...
    // 1. 检查是否需要重启应用
    // SDK 依赖从 TapTap 启动器游戏，如果用户直接运行游戏可执行文件，此函数返回 true
...
在游戏尚未发布期间，开发者可参照以下流程完成本地调试配置，配置完成后，即可在调试阶段**无需依赖 TapTap 启动器启动游戏**，直接运行游戏可执行文件进行测试。
...
### 正版验证（买断制游戏）
* 监听 `TapEventID::GamePlayableStatusChanged` 以获取游戏主体不可玩事件（用户退款、被屏蔽等情况触发）
