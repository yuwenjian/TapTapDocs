...
{package: "com.taptap.sdk.rep", url: `https://github.com/taptap/tapsdk-unity-dist.git?path=/Rep#${v4SDKVersions.taptap.shadow}`}
]}
unitypackageModules={[
...
`dependencies {
    ...
    implementation 'com.taptap.sdk:tap-rep:${v4SDKVersions.taptap.shadow}'
}`
}</CodeBlock>
...
<CodeBlock>{` pod 'TapTapSDK/Rep', '~> ${v4SDKVersions.taptap.shadow }'`}</CodeBlock>
...
<CodeBlock>{` pod 'TapTapRepSDK', '~> ${v4SDKVersions.taptap.shadow }'`}</CodeBlock>
