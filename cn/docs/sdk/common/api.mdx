...
func main() {
	// 创建一个新的 http request
	req, err := http.NewRequest("GET", "https://cloud.tapapis.cn/apk/v1/upload-params?file_name=taptap.apk&client_id=tapclientid1234567", strings.NewReader("{\"key\":\"value\"}"))
	if err != nil {
...
| 字段名 | 类型   | 描述                                                                            |
| ------ | ------ | ------------------------------------------------------------------------------- |
| success   | boolean | `true` 表示请求成功，`false` 表示请求异常，参见具体业务的错误码 |
| data   | object | 成功时返回业务数据，失败时读取 data.erorr 做错误处理 |
| now    | uint64 | 服务端响应时间 |
