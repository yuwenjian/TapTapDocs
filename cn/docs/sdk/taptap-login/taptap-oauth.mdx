...
`insufficient_scope` 错误通常是由于 [移动端授权的权限范围](/sdk/taptap-login/guide/#不同的授权范围) 与 [服务端调用的 OAuth API](#api) 所需权限 `不匹配` 导致的。

例如：移动端在进行 TapTap 授权登录时申请的是 `basic_info` 权限，但服务端调用 [获取当前账户详细信息](#获取当前账户详细信息)  API 需要更高级别的权限，由于权限不足导致服务器返回该错误码。

权限匹配规则

- 移动端授权时仅申请 `basic_info` 权限，则服务端仅可调用 [获取当前账户基础信息](#获取当前账户基础信息) API。
- 移动端授权时申请 `public_profile` 权限，则服务端可调用 [获取当前账户基础信息](#获取当前账户基础信息) API 和 [获取当前账户详细信息](#获取当前账户详细信息) API。
