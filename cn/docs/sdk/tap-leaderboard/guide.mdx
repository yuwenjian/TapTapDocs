...
3. 排行榜模块依赖于 [TapTap 登录模块](/sdk/taptap-login/features)，开发者接入前应先完成 TapTap 登录的接入
...
<CodeBlock language="json">
  {`"dependencies":{
    ...
    "com.taptap.sdk.leaderboard":"${v4SDKVersions.taptap.unity}"
  }`}
</CodeBlock>
...
<CodeBlock language="json">
  {`"dependencies":{
    ...
    "com.taptap.sdk.leaderboard":"https://github.com/taptap/tapsdk-unity-dist.git?path=/Leaderboard#${v4SDKVersions.taptap.unity}"
  }`}
</CodeBlock>
...
#### 本地文件依赖
排行榜依赖于初始化和 TapTap 登录模块，使用本地文件方式添加依赖时，需先参考[集成指南](/sdk/integration-guides/ios/#sdk-获取) 和 [TapTap 登录](/sdk/taptap-login/guide/#sdk-获取)添加对应本地文件依赖项。
...
在 iOS 项目或通过 Unity 项目导出 Xcode 工程时，因排行榜业务需要访问相册保存图片，所以需添加对应权限的使用说明，具体如下：
