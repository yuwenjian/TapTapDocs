...
// 设置是否开启测试环境
TapTapLicense.setTestEnvironment(activity: Activity, isTest: Boolean)
```
...
unitypackageModules={[
    {name: "TapSDK_Core.unitypackage", desc: "TapSDK 基础库，必选"},
    {name: "Tap_License.unitypackage", desc: "TapTap 正版验证库，必选"},
]}/>
...
在 License 模块初始化后，调用 TapTapLicense.setTestEnvironment(activity:Activity, isTest:Boolean) 开启/关闭测试模式。

### 填写测试包名、测试用户

在开发者中心 > 游戏服务 > 正版验证添加测试包名，同时将测试用户的 TapTap ID 加入测试白名单。

![](https://img.tapimg.com/market/images/e313fd948828b53edcaab3a0727801dc.png)
