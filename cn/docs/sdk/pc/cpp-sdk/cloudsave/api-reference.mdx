...
**注意事项：**
- TapCloudSave_AsyncCreate()和TapCloudSave_AsyncUpdate()共享每分钟仅允许调用60次的限制，且最多仅允许10个并发调用
...
**注意事项：**
- TapCloudSave_AsyncCreate()和TapCloudSave_AsyncUpdate()共享每分钟仅允许调用60次的限制，最多仅允许10个并发调用，且同一个云存档不允许并发更新
