...
2. 参考 [成就系统功能介绍](/sdk/achievement/features/#接入说明)开通成就系统服务
...
3. 成就模块依赖于 [TapTap 登录模块](/sdk/taptap-login/features)，开发者接入前应先完成 TapTap 登录的接入
...
<CodeBlock language="json">
  {`"dependencies":{
    ...
    "com.taptap.sdk.achievement":"${v4SDKVersions.taptap.unity}"
  }`}
</CodeBlock>
...
<CodeBlock language="json">
  {`"dependencies":{
    ...
    "com.taptap.sdk.achievement":"https://github.com/taptap/tapsdk-unity-dist.git?path=/Achievement#${v4SDKVersions.taptap.unity}"
  }`}
</CodeBlock>
...
- `TapSDK_Achievement.unitypackage` (TapTapSDK 成就模块，必选)
...
<CodeBlock>{` pod 'TapTapSDK/Achievement', '~> ${v4SDKVersions.taptap.ios }'`}</CodeBlock>

2. 执行 `Pod install` 下载对应依赖文件

#### 本地文件依赖
成就系统依赖于初始化和 TapTap 登录模块，使用本地文件方式添加依赖时，需先参考[集成指南](/sdk/integration-guides/ios/#sdk-获取) 和 [TapTap 登录](/sdk/taptap-login/guide/#sdk-获取)添加对应本地文件依赖项。
...
TapTapAchievementOptions.enableToast 为 false。 详见 [成就初始化配置](#sdk-初始化)
