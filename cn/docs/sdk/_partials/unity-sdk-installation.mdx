...
SDK 支持 **Unity Package Manager** 及**本地文件导入**两种集成方式，开发者根据需求选择其中一种即可，推荐使用远程依赖。
...
{`"dependencies":{
${props.systemNpmDeps ? ("   " + props.systemNpmDeps + "\n") : ""}   ${props.npmDeps.map(dep => `"${dep.replace('com.taptap.sdk.', 'com.taptap.tapsdk.unity.')}":"${v4SDKVersions.taptap.unity}",`).join('\n   ')}
}`}
</CodeBlock>
...
<CodeBlock language="json">
{`"dependencies":{
${""}   ${props.githubDeps.map(dep => `"${dep.package.replace('com.taptap.sdk.', 'com.taptap.tapsdk.unity.')}":"${dep.url}",`).join('\n   ')}
}`}
</CodeBlock>

> 说明：自 4.7.2 起，Unity Packages 仓库已统一为大仓 [`tapsdk-unity-dist`](https://github.com/taptap/tapsdk-unity-dist)，所有包通过目录区分并使用 `?path=` 指定子目录、`#` 指定版本标签。例如 Core 模块：
>
> ```json
