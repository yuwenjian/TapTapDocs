...
由于 SDK 默认支持移动端及 PC 平台， 所以编译时需添加 `com.google.external-dependency-manager` 的移动端依赖，如果当前工程已接入该依赖库，则不需额外处理，否则需在 `Packages/manifest.json` 添加如下依赖：
...
<CodeBlock language="json">
  {`"dependencies":{
    "com.taptap.tapsdk.unity.core":"${v4SDKVersions.taptap.unity}"
  }`}
</CodeBlock>
...
<CodeBlock language="json">
  {`"dependencies":{
    "com.taptap.tapsdk.unity.core":"https://github.com/taptap/tapsdk-unity-dist.git?path=/Core#${v4SDKVersions.taptap.unity}"
}`}
</CodeBlock>
