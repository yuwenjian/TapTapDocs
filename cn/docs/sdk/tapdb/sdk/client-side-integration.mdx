...
如果只需要单独使用 TapEvent，可以导入依赖 `com.taptap.tapsdk.unity.core`。
...
| disableReflectionOAID        | 是   | 是否禁用 OAID 反射，默认为 true                              |
| disableAutoLogDeviceLogin    | 是   | 是否禁用自动上报设备登录事件，默认为 false                           |
