...
请参考集成指南（[Unity](/sdk/integration-guides/unity)、[Android](/sdk/integration-guides/android)、[iOS](/sdk/integration-guides/ios)）完成对应平台的 SDK 基础依赖配置
...
// OAID 证书
tapSdkOptions.setOaidCert("oaidCert");
// 是否禁用 OAID 反射, 默认 true
tapSdkOptions.setDisableReflectionOAID(true);
...
options.caid = "caid" // 国内 CAID 值
options.enableAdvertiserIDCollection = false // 是否开启广告商 ID (IDFA) 收集，默认为 false
...
options.caid = @"caid"; // 国内 CAID 值
options.enableAdvertiserIDCollection = NO; // 是否开启广告商 ID (IDFA) 收集，默认为 NO
...
| enableAdvertiserIDCollection | 是   | 是否开启广告商 ID （IDFA）收集，仅 iOS ，默认为 false，开启时需添加[工程配置](#idfaios)  |
...
> 
> SDK 版本 3.28.2 及以上支持 OAID 版本为 1.0.5 ~ 2.4.0； 3.15.0 ~ 3.28.0 支持 OAID 版本为 1.0.5 ~ 2.1.0； 3.14.0 及以下支持 OAID 版本为 1.0.5 ~
> 1.0.25
...
证书信息为应用通过 移动安全联盟邮箱 [msa@caict.ac.cn](mailto:msa@caict.ac.cn) 申请的 `.cert.pem` 文件内容, 该文件与包名对应。现支持两种设置方式：
