        ...
        <tbody>
            ...
            <tr>
                ...
                <td style={{verticalAlign: "top"}} >
                    ...
                    <p><strong>link</strong> 链接的邀请次数（对应「开发者中心」 -> 「商店」-> 「版本发布」-> 「测试」 -> 「封闭式测试」 -> 「添加测试用户」 -> 「玩家分享资格」 -> 「单次分享资格可使用人」配置的值）</p>
                </td>
            </tr>
...
- 如果您的内部团队无法登录游戏，可使用内部测试功能登录游戏。内部测试功能说明请看文档《[内部测试](/store/operation/test/test-skills/internaltest)》。
...
当您的游戏需要开放测试资格至 TapTap 用户自由领取时，您可使用“先到先得-定向圈人”的功能来圈选更精确的人群。目前定向圈人能力仅针对接入 TapTap 登录的游戏开放。

:::tip
特别注意：「定向圈人」功能仅支持从预约玩家中筛选满足条件的玩家发放开测通知（不论该用户是否拥有该游戏资格），所以不支持预约量级较小的游戏使用此功能。
:::

请在封闭式测试计划 >> 发放测试资格 >> 添加测试用户 >> 选择先到先得「支持定向圈人」。您可以根据游戏类型，选择相对应的用户属性，帮助游戏找到更适合的测试人群。
  ...
  <table style={{display: "table", tableLayout: "fixed", width: "100%"}}>
    ...
    <tbody>
      <tr>
        <td>设备信息</td>
        <td>
          可查看设备跑分说明[文档](/store/operation/test/test-score/)<br/>
