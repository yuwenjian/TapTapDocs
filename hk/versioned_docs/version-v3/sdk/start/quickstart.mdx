...
请登录 [TapTap 开发者中心](https://developer.taptap.io/) 注册为开发者并创建应用。

## 下载 TapTap 应用

在测试设备中下载 [TapTap 客户端](https://www.taptap.io/mobile)，测试时会唤起 TapTap 客户端授权登录。若用户设备中未安装 TapTap 客户端，则会打开 WebView 进行登录。
