...
Cloud Engine is primarily optimized for hosting stateless HTTP services. Although your application has access to the disk, there’s no guarantee that the disk will keep the files created by your application persistently. For long-term data storage, use the <Conditional brand="tds">[Data Storage](/v3/sdk/storage/features)</Conditional><Conditional brand="leancloud">[Data Storage](/v3/sdk/storage/overview/)</Conditional> service or LeanDB’s hosted [Redis](/v3/sdk/engine/database/redis/) and [MySQL](/v3/sdk/engine/database/mysql/).
