...
With  [Standard Edition](https://developer.taptap.cn/product-intro/price), there is a free quota of 30,000 API read and write requests per day.
...
Indexes can be created in **<Path to="storage" />** by selecting the corresponding Class and going to "Performance and Indexing" self-service for more details. index).
...
In this case, you can try to rebuild the index in **<Path to="storage" /> > Full text search** .
...
The file storage service of China node comes with CDN acceleration, but it does not include overseas CDN acceleration.
 ...
 can send a work order to request to enable overseas CDN acceleration (after enabling, the cost of http/https traffic for accessing files from overseas will be adjusted to RMB 0.40/GB and RMB 0.60/GB respectively).
...
First of all, please read [**Security overview**](/v3/sdk/storage/guide/security/) to understand the complete security system of data storage service. 
