...
The Base URL for the REST API (`{{host}}` in curl examples) is the app’s  API domain, which can be  viewed on the Developer Center. See [Domain](/v3/sdk/storage/guide/setup-dotnet#domain) for more information.
