...
:::tip
Before integrating Billboard’s SDK into your game, please first **enable the service** according to [Feature Introduction](../features/#feature-introduction).
:::
...
The Base URL for REST API requests (the `{{host}}` in the curl examples) is the API domain of your app. You can update it on the Developer Center.
