...
unitypackageModules={[
{name: "TapSDK_Core.unitypackage", desc: "TapSDK Core Library, Mandatory"},
]}/>
...
- Add `-ObjC` in Other Link Flag in Build Setting.
...
The certificate information is the content of the `.cert.pem` file applied by the application through the Mobile Security Alliance email [msa@caict.ac.cn](mailto:msa@caict.ac.cn), and this file corresponds to the package name. Currently, two settings are supported:
