...
When developers integrate the SDK [login module](/sdk/taptap-login/guide), an access token (Access Token) is generated after the user authorizes your application. In the event of developers integrating the SDK login module, an access token is generated subsequent to user authorisation of the application. This token is then employed for Mac Signing computations to generate the Authorisation header for authentication and secure communication. The maximum validity of a single Access Token is 30 days. The SDK automatically refreshes the Access Token each time the application is launched to ensure the continued validity of the user session.

The mobile client of the game can obtain the currently valid Access Token through the GetCurrentTapAccount method, which always returns the most recent and valid token. The Access Token obtained by the mobile client must then be uploaded to the game server as an identifier for subsequent communication with the TapTap server.
...
>  <Red> GET </Red> https://openapi.tap.io/account/basic-info/v1?client_id=xxx <br/><Blue> Authorization </Blue> mac token
...
>  <Red> GET </Red> https://openapi.tap.io/account/profile/v1?client_id=xxx <br/><Blue> Authorization </Blue> mac token
