...
开发者接入 SDK [登录模块](/sdk/taptap-login/guide)，在用户授权你的应用程序后，将会生成访问令牌（Access Token）。该令牌将用于做 Mac 签算，生成 Authorization 头，用于身份验证和安全通信。单个 Access Token 的最长有效期为 30 天。SDK 在每次应用启动时，会自动刷新 Access Token，确保用户会话的持续有效性。 游戏移动端可通过 GetCurrentTapAccount 方法获取当前有效的 Access Token，该方法始终返回最新且有效的令牌。移动端获取的 Access Token 需上传至游戏服务器，用作与 TapTap 服务器进行后续通讯的标识。
...
3. 请求 <Conditional region='cn'>`https://open.tapapis.cn/account/profile/v1`</Conditional><Conditional region='global'>`https://open.tapapis.com/account/profile/v1`</Conditional> ， header 携带 `mac token`。
...
>  <Red> GET </Red> <Conditional region='cn'>https://open.tapapis.cn/account/basic-info/v1?client_id=xxx</Conditional><Conditional region='global'>https://open.tapapis.com/account/basic-info/v1?client_id=xxx</Conditional> <br/><Blue> Authorization </Blue> mac token
...
>  <Red> GET </Red> <Conditional region='cn'>https://open.tapapis.cn/account/profile/v1?client_id=xxx</Conditional><Conditional region='global'>https://open.tapapis.com/account/profile/v1?client_id=xxx</Conditional> <br/><Blue> Authorization </Blue> mac token
...
const requestUrl = 'https://open.tapapis.com/account/profile/v1?client_id='+ client_id ;
const method = 'GET';                    
...
public class Authorization {
    public static void main(String[] args) throws IOException {
        ...
        String mac_key = "mSUQNYUGRBPXyRyW"; // mac_key
        String method = "GET";
        String request_url = "https://open.tapapis.com/account/profile/v1?client_id=" + client_id; //
        String authorization = getAuthorization(request_url, method, kid, mac_key);
        System.out.println(authorization);
        URL url = new URL(request_url);
        HttpURLConnection conn = (HttpURLConnection) url.openConnection();
...
if __name__ == '__main__':
    ...
    mac_key = "mSUQNYUGRBPXyRyW"
    client_id = "0RiAlMny7jiz086FaU"
    signature = get_mac_token_signature('open.tapapis.com', '/account/profile/v1?client_id=' + client_id,
                                        'GET', mac_key, kid)
    print(signature)
```
</Conditional>
...
public class TapLoginOAuth : MonoBehaviour
{
    private string host = "open.tapapis.com";
    private string clientId = "应用的 Client id";
    private string macKey = "客户端登录返回的 macKey 值";
    private string kid = "客户端登录返回的 kid 值";
